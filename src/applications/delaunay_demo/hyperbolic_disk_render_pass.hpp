    ...
    void post_compile(RenderContext *render_context) override {
        line_render_system = std::make_unique<PoincareDiskRenderSystem<>>(
            ...
            std::vector<VklShaderModuleInfo>{
                {(FileSystem::getExecutablePath() / "custom_shader" / "mobius_trans_shader.vert.spv").string(),
                 VK_SHADER_STAGE_VERTEX_BIT},
                {(FileSystem::getExecutablePath() / "custom_shader" / "mobius_trans_shader.frag.spv").string(),
        ...
        point_cloud_render_system = std::make_unique<PoincareDiskPointCloudRenderSystem<>>(
            ...
            std::vector<VklShaderModuleInfo>{
                {(FileSystem::getExecutablePath() / "custom_shader" / "mobius_trans_shader.vert.spv").string(),
                 VK_SHADER_STAGE_VERTEX_BIT},
                {(FileSystem::getExecutablePath() / "custom_shader" / "point_cloud_2d_shader.frag.spv").string(),
                 VK_SHADER_STAGE_FRAGMENT_BIT}});
    }
