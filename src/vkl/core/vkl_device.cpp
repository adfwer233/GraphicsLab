...
uint32_t VklDevice::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
 ...
 * 3. alloc memory with previous memory requirements
 * 4. bind buffer with the allocated memory
 *
 * @param size
 * @param usage
 * @param properties
 ...
 * @param bufferMemory
 */
void VklDevice::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties,
...
void VklDevice::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling,
...
void VklDevice::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout,
                                      VkImageLayout newLayout) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();

    transitionImageLayout(image, format, oldLayout, newLayout, commandBuffer);

    endSingleTimeCommands(commandBuffer);
}

void VklDevice::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void VklDevice::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height,
                                  uint32_t layerCount) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
VkCommandBuffer VklDevice::beginSingleTimeCommands() {
...
void VklDevice::endSingleTimeCommands(VkCommandBuffer commandBuffer) {
...
void VklDevice::createSampler(VkSampler &sampler) {
...
VkImageView VklDevice::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectMask) {
...
void VklDevice::createImageWithInfo(const VkImageCreateInfo &imageInfo, VkMemoryPropertyFlags properties,
...
VkSampleCountFlagBits VklDevice::getMaxUsableSampleCount() {
...
void VklDevice::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout,
                                      VkCommandBuffer commandBuffer) {
    ...
    if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
        ...
        barrier.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
        barrier.dstAccessMask = 0; // No specific requirements for presentation
        sourceStage = VK_PIPELINE_STAGE_TRANSFER_BIT;
        ...
        barrier.srcAccessMask = 0; // No access mask is needed for reading from the presentation engine
        barrier.dstAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
