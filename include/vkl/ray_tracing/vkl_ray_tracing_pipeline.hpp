...
struct VklRayTracingPipeline : public VklPipeline {
  ...
  public:
    ...
    VklRayTracingPipeline(VklDevice &device, std::vector<VklShaderModuleInfo> shaderInfos,
                          VklRayTracingPipelineShaderPath shaderPath)
        : VklPipeline(device) {
        ...
        // Ray tracing shaders require SPIR-V 1.4, so we need to set the appropriate target environment for the glslang
        // compiler
        ...
        */
        ...
        vkCreateRayTracingPipelinesKHR(device_.device(), VK_NULL_HANDLE, VK_NULL_HANDLE, 1,
                                       &raytracing_pipeline_create_info, nullptr, &rayTracingPipeline);
    }
