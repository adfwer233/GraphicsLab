...
class PathTracingComputeModel {
  ...
  public:
    ...
    PathTracingComputeModel(VklDevice &device, SceneTree::VklSceneTree &scene) : device_(device), scene_(scene) {
        SceneTreeBvh bvh(scene);
        auto bvhTree = bvh.createGPUBVHTree();

        const int n = 1024;
        const int m = 1024;

        ubo.cameraPosition = scene.active_camera->camera.position;
        ubo.cameraUp = scene.active_camera->camera.camera_up_axis;
        ubo.cameraFront = scene.active_camera->camera.camera_front;
        ubo.cameraZoom = scene.active_camera->camera.zoom;
        ubo.currentSample = 0;
        ...
        auto materialBuffer = new VklBuffer(device, sizeof(SceneTree::MaterialData), materialNum,
                                            VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                                            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

        device.copyBuffer(stagingBuffer2.getBuffer(), materialBuffer->getBuffer(),
                          sizeof(SceneTree::MaterialData) * materialNum);
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, VkImage targetTexture, VkImage accumulationTexture,
        ...
        VkImageMemoryBarrier resultTranGen2Dst = VklImageUtils::transferDstToColorBarrier(renderOutputImage);
