...
struct ColorTextureResource: public Resource {
    ...
    VklTexture* getTexture() { return texture_.get(); }
    VklTexture* get_resolved_texture() {return resolved_texture_.get(); }
    ...
    void create_instance(VklDevice &device_, int texWidth, int texHeight, int texChannels,
        ...
        texture_ = std::make_unique<VklTexture>(device_, texWidth, texHeight, texChannels, usage, layout, format, samples);
        if (samples != VK_SAMPLE_COUNT_1_BIT) {
            resolved_texture_ = std::make_unique<VklTexture>(device_, texWidth, texHeight, texChannels, usage, layout, format, VK_SAMPLE_COUNT_1_BIT);
        }
    }
