...
class HashGrid {
  public:
    explicit HashGrid(float cell_size) : cell_size_(cell_size), inv_cell_size_(1.0f / cell_size) {
    }
    ...
    const Ball &getBall(int id) const {
        return balls_[id];
    }

  private:
    GridIndex cellIndex(const glm::vec3 &p) const {
        return GridIndex{static_cast<int>(std::floor(p.x * inv_cell_size_)),
