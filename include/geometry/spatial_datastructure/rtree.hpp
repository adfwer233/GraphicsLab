...
template <size_t dim, typename AttachmentType> class RTree {
    ...
    struct Sphere {
        ...
        bool contains(const PointType& p) const { return distance(center, p) < radius; }

        static double distance(const PointType& a, const PointType& b) { return glm::distance(a, b); }
    ...
    struct Node {
        ...
        Node(Sphere* s = new Sphere(), AttachmentType p = AttachmentType{}, Node* parent = nullptr, double tol = 1e-6): mbr_(s), point_(p), parent_(parent), tol_(tol) {}
  ...
  private:
    ...
    void split(Node* node, Node*& newNode) {
        ...
        newNode->children_.insert(newNode->children_.end(), std::make_move_iterator(node->children_.begin() + splitIndex), std::make_move_iterator(node->children_.end()));
        ...
        if(root_->children_.size() > 0) {
            ...
            while(!stack.empty()) {
                ...
                if(current->mbr_->contains(point)) {
                    if(current->children_.empty() and same_point(current->mbr_->center, point, (current->tol_ + point_tol) / 2)) {
                        return current;
                    }
