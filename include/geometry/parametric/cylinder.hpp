...
namespace GraphicsLab::Geometry {
    struct Cylinder: public ParamSurface {
        ...
        explicit Cylinder(const PointType& base_point, const VectorType& direction, const VectorType& radius, double length, double radius_ratio = 1.0)
        ...
        std::pair<PointType, ParamType> project(const PointType point) const override {
            ...
            if (std::abs(glm::length(radius_) - glm::length(minor_radius_)) < 1e-6) {
                ...
                return {base_point_ + glm::length(radius_) * glm::normalize(base_plane_proj - base_point_) + direction_ * v, {theta, v}};
            } else {
